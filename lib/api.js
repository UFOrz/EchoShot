...
import { PRESETS, apiBaseUrlError, isRunningHubPreset, isRunningHubTextToImageModel, supportsImageEdit } from './settings.js';
...
export async function normalizeImageBlob(blob, maxDim = 1024, { forceReencode = false } = {}) {
  ...
  let out = blob;
  if (forceReencode || scale < 1 || !/^image\/(jpeg|png|webp)$/.test(blob.type)) {
...
export function reverseRequestTimeoutMs(cfg) {
  if (isRunningHubPreset(cfg?.preset) || cfg?.preset === 'runninghub_llm') return 300000;
  try {
    if (['llm.runninghub.ai', 'llm.runninghub.cn'].includes(new URL(cfg?.baseUrl).hostname)) return 300000;
...
async function imageResultFromUrl(url, revisedPrompt = '', {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      return imageResultFromBlob(await responseBlobWithLimit(
        response,
        GENERATED_IMAGE_MAX_DOWNLOAD_SIZE,
        '生成结果超过 64 MiB，已停止下载'
      ), revisedPrompt);
    } catch (error) {
...
const GENERATED_IMAGE_MAX_DOWNLOAD_SIZE = 64 * 1024 * 1024;
const RUNNINGHUB_WORKFLOW_MAX_DOWNLOAD_SIZE = 128 * 1024 * 1024;

export async function responseBlobWithLimit(
  response,
  maxBytes = RUNNINGHUB_WORKFLOW_MAX_DOWNLOAD_SIZE,
  errorMessage = 'RunningHUB 工作流生成结果过大，已停止下载'
) {
  const sizeError = () => {
  ...
  if (declaredSize > maxBytes) throw sizeError();
...
export function qianwenImageSize(ratio = '1:1', resolution = '1k') {
  ...
  if (highResolution) {
    ...
    if (ratio === '4:3') return '2368*1728';
    if (ratio === '3:4') return '1728*2368';
  ...
  if (ratio === '4:3') return '1280*960';
  if (ratio === '3:4') return '960*1280';
...
export function bailianTokenPlanImageSize(ratio = '1:1') {
  ...
  if (ratio === '4:3') return '1472*1104';
  if (ratio === '3:4') return '1104*1472';
  return '1024*1024';
}
...
async function openRouterCapabilitiesForRequest(cfg) {
  if (cfg.imageCapabilities && typeof cfg.imageCapabilities === 'object') return cfg.imageCapabilities;
  const cacheKey = `${String(cfg.baseUrl || '').replace(/\/+$/, '')}\n${cfg.model}`;
  if (!openRouterImageCapabilityCache.has(cacheKey)) {
    const request = requestJson(joinUrl(cfg.baseUrl, '/images/models'), cfg.apiKey, { timeoutMs: 20000 })
      .then((data) => {
        const item = (Array.isArray(data?.data) ? data.data : []).find((candidate) => candidate?.id === cfg.model);
        if (!item) throw new Error(`OpenRouter 图片模型目录中未找到“${cfg.model}”`);
        return item.supported_parameters && typeof item.supported_parameters === 'object'
          ? item.supported_parameters
          : {};
      })
      .catch((error) => {
        openRouterImageCapabilityCache.delete(cacheKey);
        throw new Error(`无法确认 OpenRouter 模型能力：${error?.message || error}`);
      });
    openRouterImageCapabilityCache.set(cacheKey, request);
  }
  return openRouterImageCapabilityCache.get(cacheKey);
}
...
export function openRouterImageRequestBody({
  ...
  const normalizedResolution = normalizedImageResolution(resolution).toUpperCase();
  const descriptorAccepts = (descriptor, value) => {
    if (!descriptor || typeof descriptor !== 'object') return false;
    if (descriptor.type !== 'enum' || !Array.isArray(descriptor.values)) return true;
    return descriptor.values.some((candidate) => String(candidate).toUpperCase() === String(value).toUpperCase());
  };
  if (capabilities && typeof capabilities === 'object') {
    const ratioDescriptor = capabilities.aspect_ratio;
    if (!ratioDescriptor && normalizedRatio !== '1:1') {
      throw new Error(`当前 OpenRouter 模型未声明支持比例 ${normalizedRatio}`);
    }
    if (ratioDescriptor && !descriptorAccepts(ratioDescriptor, normalizedRatio)) {
      throw new Error(`当前 OpenRouter 模型不支持比例 ${normalizedRatio}`);
    }
    if (ratioDescriptor) body.aspect_ratio = normalizedRatio;

    const resolutionDescriptor = capabilities.resolution;
    if (!resolutionDescriptor && normalizedResolution !== '1K') {
      throw new Error(`当前 OpenRouter 模型未声明支持分辨率 ${normalizedResolution}`);
    }
    if (resolutionDescriptor && !descriptorAccepts(resolutionDescriptor, normalizedResolution)) {
      throw new Error(`当前 OpenRouter 模型不支持分辨率 ${normalizedResolution}`);
    }
    if (resolutionDescriptor) body.resolution = normalizedResolution;
  } else {
    body.aspect_ratio = normalizedRatio;
  ...
  if (/^openai\//i.test(String(model))) {
    if (!capabilities || capabilities.n) body.n = 1;
    if (!capabilities || capabilities.quality) body.quality = normalizedImageQuality(quality);
  }
...
export async function generateOpenRouterImage({
  ...
  const capabilities = await openRouterCapabilitiesForRequest(cfg);
  const data = await postJson(
...
export function runningHubNeedsSource(model, imageCapabilities = null) {
  const params = imageCapabilities?.runningHubParams;
  if (Array.isArray(params)) {
    return params.some((param) => param?.required === true && param?.type === 'IMAGE')
      || /(image-to-image|image-edit|\/edit(?:$|-))/i.test(String(model || ''));
  }
...
function runningHubRegistryOption(param, preferred, fallback = '') {
  const options = Array.isArray(param?.options) ? param.options : [];
  const match = options.find((item) => String(item).toLowerCase() === String(preferred).toLowerCase());
  if (match !== undefined) return match;
  if (param?.defaultValue !== undefined && param.defaultValue !== null) return param.defaultValue;
  return fallback || options[0];
}

function runningHubRegistrySize(param, ratio, configuredSize) {
  const options = Array.isArray(param?.options) ? param.options : [];
  const configured = String(configuredSize || '').replace(/x/i, '*');
  const exact = options.find((item) => String(item).replace(/x/i, '*') === configured);
  if (exact !== undefined) return exact;
  const ratioMatch = String(ratio || '').match(/^(\d+(?:\.\d+)?):(\d+(?:\.\d+)?)$/);
  const targetRatio = Number(ratioMatch?.[1] || 1) / Number(ratioMatch?.[2] || 1);
  const candidates = options.map((item) => {
    const match = String(item).match(/(\d+)\s*[*x:]\s*(\d+)/i);
    return match ? { item, distance: Math.abs(Number(match[1]) / Number(match[2]) - targetRatio) } : null;
  }).filter(Boolean).sort((left, right) => left.distance - right.distance);
  return candidates[0]?.item ?? runningHubRegistryOption(param, '', configuredSize || '1024*1024');
}

function runningHubRegistryRequestBody({
  params,
  prompt,
  ratio,
  size,
  ...
  const body = {};
  const { width, height } = runningHubSeedreamDimensions(ratio, size);
  for (const param of params) {
    const key = String(param?.fieldKey || '').trim();
    if (!key) continue;
    let value;
    if (key === 'prompt') value = prompt;
    else if (param?.type === 'IMAGE') {
      const primaryImageField = ['image', 'imageUrl', 'imageUrls'].includes(key);
      if (imageUrl && (param?.required === true || primaryImageField)) {
        value = key === 'imageUrls'
          ? [imageUrl, referenceImageUrl].filter(Boolean)
          : imageUrl;
      }
    } else if (key === 'aspectRatio') {
      value = runningHubRegistryOption(param, ratio || '1:1', ratio || '1:1');
    } else if (key === 'resolution') {
      value = runningHubRegistryOption(param, normalizedImageResolution(resolution), normalizedImageResolution(resolution));
    } else if (key === 'size') {
      value = runningHubRegistrySize(param, ratio, size);
    } else if (key === 'width') value = width;
    else if (key === 'height') value = height;
    else if (key === 'outputFormat') value = runningHubRegistryOption(param, 'png', 'png');
    else if (param?.defaultValue !== undefined && param.defaultValue !== null) {
      value = structuredClone(param.defaultValue);
    }
    if (value !== undefined && value !== null && value !== '') body[key] = value;
    if (param?.required === true && !(key in body)) {
      if (param?.type === 'IMAGE') {
        throw new Error('该 RunningHUB 模型需要来源图片，请先点击网页图片上的魔法按钮');
      }
      throw new Error(`RunningHUB 模型缺少必填参数：${key}`);
    }
  }
  return body;
}

export function runningHubRequestBody({ model, prompt, ratio, size = '', imageUrl, referenceImageUrl = '', resolution = '1k', imageCapabilities = null }) {
  if (Array.isArray(imageCapabilities?.runningHubParams)) {
    return runningHubRegistryRequestBody({
      params: imageCapabilities.runningHubParams,
      prompt,
      ratio,
      size,
      imageUrl,
  ...
  if (runningHubNeedsSource(model, imageCapabilities)) {
...
function runningHubMetaAttribute(tag, name) {
  const match = tag.match(new RegExp(`(?:^|\\s)${name}\\s*=\\s*(?:"([^"]*)"|'([^']*)')`, 'i'));
  return match?.[1] ?? match?.[2] ?? '';
}

function decodeRunningHubTitle(value) {
...
export function runningHubWorkflowTitleFromHtml(html) {
  const meta = (String(html || '').match(/<meta\b[^>]*>/gi) || []).find((tag) => (
    runningHubMetaAttribute(tag, 'property').toLowerCase() === 'og:title'
    || runningHubMetaAttribute(tag, 'name').toLowerCase() === 'og:title'
  ));
  const title = decodeRunningHubTitle(runningHubMetaAttribute(meta || '', 'content'))
    .replace(/\s*[-–—|]\s*RunningHub ComfyUI Workflow\s*$/i, '')
    .replace(/\s+/g, ' ')
    .trim();
  if (!title || title.length > 160 || /^RunningHub(?:\s+ComfyUI(?:\s+Workflow)?)?$/i.test(title)) return '';
  return title;
}

export async function fetchRunningHubWorkflowTitle(preset, model) {
  const workflowId = runningHubWorkflowId(model);
  if (!isRunningHubPreset(preset) || !workflowId) return '';
  const origin = `https://www.runninghub.${preset === 'runninghub_cn' ? 'cn' : 'ai'}`;
  const url = `${origin}/post/${workflowId}`;
  const ctrl = new AbortController();
  ...
  try {
    ...
    if (response.url) {
      ...
      if (finalUrl.origin !== origin || finalUrl.pathname.replace(/\/+$/, '') !== `/post/${workflowId}`) return '';
    }
    const contentType = response.headers?.get?.('content-type') || '';
    const contentLength = Number(response.headers?.get?.('content-length') || 0);
    if ((contentType && !/text\/html/i.test(contentType)) || contentLength > 1024 * 1024) return '';
    const html = await response.text();
    return html.length <= 1024 * 1024 ? runningHubWorkflowTitleFromHtml(html) : '';
  } catch {
    // 私有/失效工作流、网络失败或超时都不妨碍用户手动添加。
    return '';
  } finally {
    clearTimeout(timer);
  }
...
export function findRunningHubWorkflowInputNodes(workflow) {
  ...
  const actualFieldName = (nodeId, node, key, spec) => {
    const inputs = node?.inputs;
    const names = inputs && typeof inputs === 'object' && !Array.isArray(inputs)
      ? Object.keys(inputs)
      : [];
    if (Object.hasOwn(inputs || {}, spec.fieldName)) return spec.fieldName;
    const compatible = key === 'prompt'
      ? ['prompt', 'positive', 'input_text', 'string', 'value']
      : [key];
    const candidates = compatible.filter((name) => typeof inputs?.[name] === (key === 'prompt' ? 'string' : 'number')
      || (key !== 'prompt' && /^\d+$/.test(String(inputs?.[name] ?? ''))));
    if (candidates.length === 1) return candidates[0];
    const available = names.length ? names.join('、') : '无';
    throw new Error(`RunningHUB 工作流节点 ${nodeId}（${spec.description}）没有可确定的输入字段；现有字段：${available}。请检查或重新导出工作流 API JSON`);
  };
  const found = {};
  for (const [nodeId, node] of Object.entries(nodes)) {
    ...
    for (const [key, spec] of Object.entries(specs)) {
      if (title === spec.marker) {
        found[key] = {
          nodeId,
          fieldName: actualFieldName(nodeId, node, key, spec),
...
export async function generateRunningHubImage({
  ...
  const submit = await postJson(
    ...
    runningHubRequestBody({
      ...
      referenceImageUrl,
      resolution,
      imageCapabilities: cfg.imageCapabilities
    }),
    120000
  );
...
export async function generateRunningHubWorkflowImage({
  ...
  const submitFailure = runningHubFailure(submit);
  if (submitFailure) {
    if (/NODE_INFO_MISMATCH|field_not_found_in_node_inputs/i.test(submitFailure)) {
      throw new Error(`RunningHUB 工作流节点字段与平台当前定义不一致：${submitFailure}。请在 RunningHUB 重新导出该工作流的 API JSON，核对报错节点的 inputs`);
    }
    throw new Error('RunningHUB 工作流提交失败：' + submitFailure);
  }
...
export async function listApiMartModelCatalog() {
  const ctrl = new AbortController();
  ...
  try {
    ...
    const uniqueVisionModels = [...new Set(visionModels)];
    const uniqueImageModels = [...new Set(imageModels)];
    const imageEditModels = uniqueImageModels.filter((model) => supportsImageEdit({ preset: 'apimart', model }));
    const modelKinds = {};
    for (const model of uniqueVisionModels) modelKinds[model] = ['vision'];
    for (const model of uniqueImageModels) {
      modelKinds[model] = [...new Set([
        ...(modelKinds[model] || []),
        'text-to-image',
        ...(imageEditModels.includes(model) ? ['image-edit'] : [])
      ])];
    }
    return {
      visionModels: uniqueVisionModels,
      imageModels: uniqueImageModels,
      imageEditModels,
      modelKinds,
      models: [...new Set([...uniqueVisionModels, ...uniqueImageModels])]
    };
  } finally {
    clearTimeout(timer);
  }
...
async function listApiMartModels() {
  return (await listApiMartModelCatalog()).models;
}
...
export async function listOpenRouterModels(cfg) {
  ...
  const imageCapabilities = Object.fromEntries(imageItems
    .filter((item) => item?.id && item?.supported_parameters && typeof item.supported_parameters === 'object')
    .map((item) => [item.id, item.supported_parameters]));
  const imageEditModels = imageItems
    ...
    .map((item) => item?.id)
    .filter(Boolean);
  const uniqueVisionModels = [...new Set(visionModels)].sort();
  const uniqueImageModels = [...new Set(imageModels)].sort();
  const uniqueImageEditModels = [...new Set(imageEditModels)].sort();
  const modelKinds = {};
  for (const model of uniqueVisionModels) modelKinds[model] = ['vision'];
  for (const model of uniqueImageModels) {
    modelKinds[model] = [...new Set([
      ...(modelKinds[model] || []),
      'text-to-image',
      ...(uniqueImageEditModels.includes(model) ? ['image-edit'] : [])
    ])];
  }
  return {
    visionModels: uniqueVisionModels,
    imageModels: uniqueImageModels,
    imageEditModels: uniqueImageEditModels,
    imageCapabilities,
    modelKinds,
    models: [...new Set([...uniqueVisionModels, ...uniqueImageModels])].sort()
  };
}

const RUNNINGHUB_PUBLIC_REGISTRY_URLS = [
  'https://raw.githubusercontent.com/HM-RunningHub/ComfyUI_RH_OpenAPI/main/developer-kit/model-registry.public.json',
  'https://cdn.jsdelivr.net/gh/HM-RunningHub/ComfyUI_RH_OpenAPI@main/developer-kit/model-registry.public.json'
];

async function fetchPublicJson(url, timeoutMs = 20000) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
...
async function fetchRunningHubRegistry() {
  const errors = [];
  for (const url of RUNNINGHUB_PUBLIC_REGISTRY_URLS) {
    try {
  ...
  throw new Error(errors.join('；') || '无法读取官方模型注册表');
}

function runningHubRegistryParam(param) {
  const fieldKey = String(param?.fieldKey || '').trim();
  if (!fieldKey || fieldKey.length > 100 || ['__proto__', 'constructor', 'prototype'].includes(fieldKey)) return null;
  const out = {
    fieldKey,
  ...
  if (param?.defaultValue !== undefined && param.defaultValue !== null) {
    out.defaultValue = structuredClone(param.defaultValue);
  }
  const options = (Array.isArray(param?.options) ? param.options : [])
...
export function runningHubModelsFromRegistry(registry) {
  const items = Array.isArray(registry?.models) ? registry.models : [];
  const imageModels = [];
  ...
  for (const item of items) {
    ...
    const params = (Array.isArray(item?.params) ? item.params : [])
      .map(runningHubRegistryParam)
      .filter(Boolean);
    if (
      !endpoint
      || endpoint.length > 300
      || ['__proto__', 'constructor', 'prototype'].includes(endpoint)
      || item?.output_type !== 'image'
      || /deprecated/i.test(endpoint)
    ) continue;
    if (!params.some((param) => param.fieldKey === 'prompt')) continue;
    const generatedFields = new Set(['prompt', 'aspectRatio', 'resolution', 'size', 'width', 'height', 'outputFormat']);
    const unsupportedRequired = params.some((param) => (
      param.required
      && param.type !== 'IMAGE'
      && !generatedFields.has(param.fieldKey)
      && param.defaultValue === undefined
    ));
    if (unsupportedRequired) continue;
    imageModels.push(endpoint);
    imageCapabilities[endpoint] = { runningHubParams: params };
    const needsImage = params.some((param) => param.required && param.type === 'IMAGE');
    if (needsImage || /(image-to-image|image-edit|\/edit(?:$|-))/i.test(endpoint)) {
      imageEditModels.push(endpoint);
      modelKinds[endpoint] = ['image-edit'];
    } else {
      modelKinds[endpoint] = ['text-to-image'];
    }
    const alias = String(item?.name_cn || item?.display_name || '').replace(/^RH\s+/i, '').trim();
    if (alias && alias !== endpoint) modelAliases[endpoint] = alias;
  }
  return {
...
export async function listRunningHubModels(cfg) {
  const llmUrl = `https://llm.runninghub.${cfg?.preset === 'runninghub_cn' ? 'cn' : 'ai'}/v1/models`;
  const [llmResult, registryResult] = await Promise.allSettled([
    requestJson(llmUrl, cfg?.apiKey || '', { timeoutMs: 20000 }),
    fetchRunningHubRegistry()
  ]);
  if (llmResult.status === 'rejected' && registryResult.status === 'rejected') {
    throw new Error(
      `RunningHUB 获取模型失败：${llmResult.reason?.message || ''}` +
      `${llmResult.reason && registryResult.reason ? '；' : ''}${registryResult.reason?.message || ''}`
    );
  }
  const llmItems = llmResult.status === 'fulfilled' && Array.isArray(llmResult.value?.data)
    ? llmResult.value.data
    : [];
  const visionModels = llmItems
    .filter((item) => item?.capabilities?.vision === true)
    .map((item) => String(item?.id || '').trim())
    .filter(Boolean);
  const registryModels = registryResult.status === 'fulfilled'
    ? runningHubModelsFromRegistry(registryResult.value)
    : { imageModels: [], imageEditModels: [], imageCapabilities: {}, modelAliases: {}, modelKinds: {} };
  const models = [...new Set([...visionModels, ...registryModels.imageModels])].sort();
  if (!models.length) {
    throw new Error('RunningHUB 官方目录未返回可用的视觉或生图模型');
  }
  return {
    visionModels: [...new Set(visionModels)].sort(),
    imageModels: registryModels.imageModels,
    imageEditModels: registryModels.imageEditModels,
    imageCapabilities: registryModels.imageCapabilities,
    modelAliases: registryModels.modelAliases,
    modelKinds: {
      ...Object.fromEntries(visionModels.map((model) => [model, ['vision']])),
      ...registryModels.modelKinds
    },
    models
  };
}
...
export async function listModels(cfg) {
  ...
  if (isRunningHubPreset(cfg?.preset)) return (await listRunningHubModels(cfg)).models;
  ...
  try {
    ...
    try {
      ...
      throw new Error('模型列表接口返回的内容不是有效 JSON（可能是网页），请检查 Base URL 或选择对应的内置接口模板');
    }
