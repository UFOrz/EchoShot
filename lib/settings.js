...
export const PRESETS = {
  ...
  runninghub: {
    ...
    imageModels: [
      'rhart-image-n-g31-flash-lite/text-to-image',
      'rhart-image-n-g31-flash-lite/image-to-image',
      'rhart-image-n-g31-flash-lite-official/text-to-image',
      'rhart-image-n-g31-flash-lite-official/image-to-image',
      ...
      'rhart-image/z-image/turbo',
      'rhart-image/f-2-klein-4b/text-to-image',
      ...
      'workflow/2083186745274159105',
      'workflow/2083186756980461570',
      'workflow/2081203973859631106'
    ],
    modelAliases: {
      'rhart-image-n-g31-flash-lite/text-to-image': 'nano-banana2-lite-t2i-OEM',
      'rhart-image-n-g31-flash-lite/image-to-image': 'nano-banana2-lite-i2i-OEM',
      'rhart-image-n-g31-flash-lite-official/text-to-image': 'nano-banana2-lite-t2i-official',
      'rhart-image-n-g31-flash-lite-official/image-to-image': 'nano-banana2-lite-i2i-official',
      'rhart-image-g-2-official/text-to-image': 'image2-t2i-official',
      'rhart-image-g-2-official/image-to-image': 'image2-i2i-official',
      'rhart-image-g-2/text-to-image': 'image2-t2i-OEM',
      'rhart-image-g-2/image-to-image': 'image2-i2i-OEM',
      'rhart-image-n-g31-flash/text-to-image': 'nano-banana2-t2i-OEM',
      'rhart-image-n-g31-flash/image-to-image': 'nano-banana2-i2i-OEM',
      'rhart-image-n-pro/text-to-image': 'nano-banana-pro-t2i-OEM',
      'rhart-image-n-pro/edit': 'nano-banana-pro-i2i-OEM',
      'rhart-image-n-g31-flash-official/text-to-image': 'nano-banana2-t2i-official',
      'rhart-image-n-g31-flash-official/image-to-image': 'nano-banana2-i2i-official',
      'rhart-image-n-pro-official/text-to-image': 'nano-banana-pro-t2i-official',
      'rhart-image-n-pro-official/edit': 'nano-banana2-i2i-official',
      'seedream-v5-pro/text-to-image': 'seedreamv5pro-t2i',
      'seedream-v5-pro/image-to-image': 'seedreamv5pro-i2i',
      'dola-Seedream-5.0-pro/text-to-image': 'seedreamv5pro-t2i',
      'dola-Seedream-5.0-pro/image-to-image': 'seedreamv5pro-i2i',
      'youchuan/text-to-image-v7': 'MidjourneyV7',
      'youchuan/text-to-image-v81': 'MidjourneyV8.1',
      'youchuan/text-to-image-v82': 'MidjourneyV8.2',
      'rhart-image/z-image/turbo': 'z-image-turbo',
      'rhart-image/f-2-klein-4b/text-to-image': 'FLUX2-Klein',
      'rhart-image/f-2-dev/text-to-image': 'FLUX2-dev',
      'workflow/2078442367867637761': 'Z-Image-Turbo-workflow',
      'workflow/2078448276088655873': 'MoodyProMixZIT-V10-workflow',
      'workflow/2080930269801361409': 'Z-Image-Turbo-LoRA-workflow',
      'workflow/2080965696364064770': 'Z-Image-Turbo-LoRA2-workflow',
      'workflow/2083186745274159105': 'MoodyKreaMix4-workflow',
      'workflow/2083186756980461570': 'Krea2TurboRealistic-workflow',
      'workflow/2081203973859631106': 'Krea2Turbo-workflow'
    }
  },
  custom: { label: '自定义（OpenAI 兼容接口）', baseUrl: '', visionModels: [], imageModels: [] }
};

// 国内站不预置模型：用户填写 Key 后由设置页从 RunningHUB 官方
// LLM 目录和标准模型注册表实时获取，避免国内外站模型集不一致。
PRESETS.runninghub_cn = {
  label: 'RunningHUB 国内站',
  baseUrl: 'https://www.runninghub.cn',
  getKeyUrl: 'https://www.runninghub.cn?inviteCode=4089dae7',
  affiliateLink: true,
  visionModels: [],
  imageModels: [],
  imageEditModels: [],
...
export function isRunningHubPreset(preset = '') {
  return preset === 'runninghub' || preset === 'runninghub_cn';
}

export function presetIdFromBaseUrl(value) {
  const text = String(value || '').trim();
  if (!text) return '';
  let url;
  try {
  ...
  const exact = Object.entries(PRESETS).find(([id, preset]) => (
    id !== 'custom' && preset.baseUrl.replace(/\/+$/, '') === normalized
  ));
  if (exact) return exact[0];

  const path = url.pathname.replace(/\/+$/, '') || '/';
  if (
    ['/', '/openapi/v2'].includes(path)
    && ['www.runninghub.ai', 'www.runninghub.cn'].includes(url.hostname)
  ) {
    return url.hostname.endsWith('.cn') ? 'runninghub_cn' : 'runninghub';
  }
  if (path === '/v1' && ['llm.runninghub.ai', 'llm.runninghub.cn'].includes(url.hostname)) {
    return url.hostname.endsWith('.cn') ? 'runninghub_cn' : 'runninghub';
  }
  return '';
}
...
const MODEL_KINDS = ['vision', 'text-to-image', 'image-edit'];
...
function cleanModelKinds(value, models) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  const knownModels = new Set(models);
  const out = {};
  for (const [modelValue, kindsValue] of Object.entries(value)) {
    const model = String(modelValue || '').trim();
    if (!knownModels.has(model) || ['__proto__', 'constructor', 'prototype'].includes(model)) continue;
    const kinds = cleanModels(kindsValue).filter((kind) => MODEL_KINDS.includes(kind));
    if (kinds.length) out[model] = kinds;
  }
  return out;
}
...
export function setModelAliasIfEmpty(platform, model, alias) {
  const value = String(alias || '').trim();
  if (!value || !platform?.models?.includes(model) || String(platform.modelAliases?.[model] || '').trim()) return false;
  platform.modelAliases ||= {};
  platform.modelAliases[model] = value;
  return true;
}

export function buildModelAliasIndex(settings) {
  const byPlatformId = new Map();
  const byProvider = new Map();
  const byModel = new Map();
  const ambiguousModels = new Set();
  for (const platform of settings?.platforms || []) {
    for (const [modelValue, aliasValue] of Object.entries(platform.modelAliases || {})) {
      const model = String(modelValue || '').trim();
      const alias = String(aliasValue || '').trim();
      if (!model || !alias) continue;
      if (platform.id) byPlatformId.set(`${platform.id}\n${model}`, alias);
      if (platform.name) byProvider.set(`${platform.name}\n${model}`, alias);
      if (ambiguousModels.has(model)) continue;
      if (!byModel.has(model) || byModel.get(model) === alias) {
        byModel.set(model, alias);
      } else {
  ...
  return { byPlatformId, byProvider, byModel };
}

export function recordModelDisplayName(index, record) {
  const model = String(record?.model || '').trim();
  if (!model) return '';
  const platformId = String(record?.platformId || '').trim();
  const provider = String(record?.provider || '').trim();
  return (platformId ? index?.byPlatformId?.get(`${platformId}\n${model}`) : '')
    || (provider ? index?.byProvider?.get(`${provider}\n${model}`) : '')
...
function normalizePlatform(platform, index = 0) {
  ...
  if (platform?.imageCapabilities && typeof platform.imageCapabilities === 'object' && !Array.isArray(platform.imageCapabilities)) {
    for (const [model, capabilities] of Object.entries(platform.imageCapabilities)) {
      if (!models.includes(model) || !capabilities || typeof capabilities !== 'object' || Array.isArray(capabilities)) continue;
      if (['__proto__', 'constructor', 'prototype'].includes(model)) continue;
      imageCapabilities[model] = structuredClone(capabilities);
    }
  }
  return {
    ...
    modelKinds: cleanModelKinds(platform?.modelKinds, models),
    imageCapabilities,
...
function normalizeSettings(raw = {}) {
  ...
  if ((raw.presetRevision || 0) < PRESET_REVISION) {
    ...
    // revision 30 增加 RunningHUB 国内站模板。旧版会把用户手动填入的
    // .cn 官方地址误识别为 OpenAI 兼容自定义接口，这里只修复精确的
    // RunningHUB 官方根地址/API 地址，不改动其他自定义平台。
    if ((raw.presetRevision || 0) < 30) {
      platforms = platforms.map((platform) => {
        if (platform.preset !== 'custom' || presetIdFromBaseUrl(platform.baseUrl) !== 'runninghub_cn') {
          return platform;
        }
        return {
          ...platform,
          preset: 'runninghub_cn',
          name: PRESETS.runninghub_cn.label,
          baseUrl: PRESETS.runninghub_cn.baseUrl
        };
      });
    }
    // revision 31 取消 RunningHUB 国内站的静态默认模型。清理
    // revision 30 写入的国际站镜像列表，随后由用户点击“自动获取模型”
    // 从国内站和官方标准模型注册表重新载入。
    if ((raw.presetRevision || 0) < 31) {
      platforms = platforms.map((platform) => (
        platform.preset === 'runninghub_cn'
          ? normalizePlatform({
              ...platform,
              models: [],
              modelAliases: {},
    ...
    platforms = platforms.map((platform) => {
      ...
      return normalizePlatform({
        ...
        modelAliases: { ...desired.modelAliases, ...(platform.modelAliases || {}) },
  ...
  const out = {
    ...
    sizeMap: Object.fromEntries(RATIOS.map((ratio) => {
      const value = String(raw.sizeMap?.[ratio] || '').trim().toLowerCase();
      return [ratio, imageSizeError(value) ? DEFAULT_SETTINGS.sizeMap[ratio] : value];
    })),
...
export function applyRunningHubModelCatalog(platform, catalog, { replace = false } = {}) {
  const fetchedModels = cleanModels(catalog?.models);
  const models = replace
    ? fetchedModels
    : cleanModels([...(platform?.models || []), ...fetchedModels]);
  const inCatalog = (items) => cleanModels(items).filter((model) => models.includes(model));
  const fetchedAliases = cleanModelAliases(catalog?.modelAliases, models);
  const currentAliases = cleanModelAliases(platform?.modelAliases, models);
  const currentKinds = cleanModelKinds(platform?.modelKinds, models);
  const fetchedKinds = cleanModelKinds(catalog?.modelKinds, models);
  const imageCapabilities = {};
  const capabilitySources = replace
    ? [catalog?.imageCapabilities]
    : [platform?.imageCapabilities, catalog?.imageCapabilities];
  ...
  return {
    ...platform,
    models,
    // 在线目录只提供能力提示，不代替用户勾选“反推/生图”。
    visionModels: inCatalog(platform?.visionModels),
    imageModels: inCatalog(platform?.imageModels),
    imageEditModels: replace
      ? inCatalog(catalog?.imageEditModels)
      : inCatalog([...(platform?.imageEditModels || []), ...(catalog?.imageEditModels || [])]),
    modelAliases: replace ? fetchedAliases : { ...fetchedAliases, ...currentAliases },
    modelKinds: replace ? fetchedKinds : { ...currentKinds, ...fetchedKinds },
    imageCapabilities
  };
}
...
export function supportsImageEdit({ preset = '', model = '', imageEdit = false } = {}) {
  ...
  if (preset === 'atlascloud') {
    return isAtlasCloudNanoBananaEditModel(name) || /^openai\/gpt-image-2\/edit$/i.test(name);
  }
  if (isModelScopeImageEditModel({ preset, model: name })) return true;
  return /(gpt-image|image-to-image|\/edit(?:$|[/?]))/i.test(name);
}

export function isAtlasCloudNanoBananaEditModel(model = '') {
  return /^google\/(?:nano-banana|nano-banana-2|nano-banana-2-lite|nano-banana-pro)\/edit(?:-developer|-ultra)?$/i.test(String(model));
}
...
export function isRunningHubTextToImageModel(model = '') {
  const name = String(model).trim();
  return /^workflow\/\d+$/.test(name)
    || /\/text-to-image(?:$|[-/])/i.test(name)
    || name === 'rhart-image/z-image/turbo';
}

export function requiresSourceImage({ preset = '', model = '' } = {}) {
  const name = String(model);
  if (isModelScopeImageEditModel({ preset, model: name })) return true;
  if (isRunningHubPreset(preset)) {
    return !isRunningHubTextToImageModel(name);
  }
  return /(image-to-image|\/edit(?:$|[-/?]))/i.test(name);
}

export function modelCapabilityKinds(platform, model = '') {
  const name = String(model || '').trim();
  if (!name) return [];
  const preset = PRESETS[platform?.preset] || null;
  const explicitKinds = cleanModels(platform?.modelKinds?.[name])
    .filter((kind) => MODEL_KINDS.includes(kind));
  const kinds = new Set(explicitKinds);

  // 预设目录和接口返回的编辑能力属于平台元数据；用户手动勾选的
  // visionModels/imageModels 不参与推断，避免把“已启用”误当成“已确认支持”。
  if ((preset?.visionModels || []).includes(name)) kinds.add('vision');
  const knownImageModel = (preset?.imageModels || []).includes(name);
  const knownImageEdit = (preset?.imageEditModels || []).includes(name)
    || (platform?.imageEditModels || []).includes(name)
    || supportsImageEdit({ preset: platform?.preset, model: name });
  if (knownImageModel && !requiresSourceImage({ preset: platform?.preset, model: name })) {
    kinds.add('text-to-image');
  }
  if (knownImageEdit) kinds.add('image-edit');

  return MODEL_KINDS.filter((kind) => kinds.has(kind));
}
...
export function resolveModelConfig(settings, type, selection) {
  ...
  const runningHub = isRunningHubPreset(platform.preset);
  const runningHubChina = platform.preset === 'runninghub_cn';
  ...
  return {
    ...
    baseUrl: runningHub
      ? (type === 'vision'
          ? `https://llm.runninghub.${runningHubChina ? 'cn' : 'ai'}/v1`
          : `https://www.runninghub.${runningHubChina ? 'cn' : 'ai'}/openapi/v2`)
      : platform.baseUrl,
    apiKey: platform.apiKey,
    model: choice.model,
    imageEdit: choice.imageEdit === true,
    imageCapabilities: platform.imageCapabilities?.[choice.model] || null
  };
}
...
export async function saveSettings(settings) {
  for (const ratio of RATIOS) {
    const error = imageSizeError(settings?.sizeMap?.[ratio]);
    if (error) throw new Error(`${ratio}：${error}`);
  }
